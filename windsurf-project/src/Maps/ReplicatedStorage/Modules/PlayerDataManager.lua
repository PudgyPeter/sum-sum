...
-- For map places, use Modules instead of Shared
local Modules = ReplicatedStorage:WaitForChild("Modules")
local GameConfig = require(Modules:WaitForChild("GameConfig"))
local DataStoreManager = require(Modules:WaitForChild("DataStoreManager"))
...
local function getDefaultData()
	-- Ensure GameConfig is loaded
	if not GameConfig then
		GameConfig = require(Modules:WaitForChild("GameConfig"))
	end
	
	-- Use direct access to avoid function call issues
	local startingGems = 1000
	local startingCoins = 500
	local startingTickets = 5
	
	-- Try to get values from GameConfig if available
	if GameConfig and GameConfig.Currency then
		startingGems = GameConfig.Currency.StartingGems or startingGems
		startingCoins = GameConfig.Currency.StartingCoins or startingCoins
		startingTickets = GameConfig.Currency.StartingTickets or startingTickets
	end
...
function PlayerDataManager.SerializePlayerData(player)
	...
	-- Serialize loadout (now stores instance UUIDs)
	...
	if loadout then
		for _, slot in ipairs(loadout:GetChildren()) do
			if slot:IsA("StringValue") then
				...
				if slotNumber then
					data.Loadout[slotNumber] = slot.Value
				end
			end
		end
	end
...
function PlayerDataManager.SetupPlayer(player)
	...
	if data.SchemaVersion == 1 or (data.Inventory and #data.Inventory > 0 and not next(data.UnitInstances or {})) then
		...
		for i, towerId in ipairs(data.Inventory or {}) do
			-- Create a new unit instance for each legacy tower
			...
			instanceFolder:SetAttribute("Rarity", "Common") -- Default, should be looked up from tower data
			instanceFolder.Parent = unitInstances
			
			-- Empty traits and relics
			local traitsFolder = Instance.new("Folder")
			traitsFolder.Name = "Traits"
			traitsFolder.Parent = instanceFolder
...
function PlayerDataManager.SetMapProgress(player, mapId, actNumber)
	print("[PlayerData] SetMapProgress called for", player.Name, "map:", mapId, "act:", actNumber)
	
	local playerData = player:FindFirstChild("PlayerData")
	if not playerData then 
		warn("[PlayerData] No PlayerData found for", player.Name)
		return false 
	end
	
	local mapProgress = playerData:FindFirstChild("MapProgress")
	if not mapProgress then 
		warn("[PlayerData] No MapProgress found for", player.Name)
		return false 
	end
	
	local currentProgress = PlayerDataManager.GetMapProgress(player, mapId)
	print("[PlayerData] Current progress for", mapId, ":", currentProgress, "new:", actNumber)
	if actNumber <= currentProgress then 
		print("[PlayerData] Not downgrading progress for", player.Name)
		return true -- Don't downgrade progress
	end
	
	local progressValue = mapProgress:FindFirstChild(mapId)
	if progressValue then
		progressValue.Value = mapId .. ":" .. actNumber
		print("[PlayerData] Updated existing progress for", mapId)
	else
		...
		print("[PlayerData] Created new progress for", mapId)
	end
	
	print("[PlayerData] Set", player.Name, "progress for", mapId, "to Act", actNumber)
	
	-- Auto-save progress to DataStore
	local DataStoreManager = require(script.Parent.DataStoreManager)
	local data = PlayerDataManager.SerializePlayerData(player)
	if data then
		DataStoreManager.SaveData(player, data)
		print("[PlayerData] Auto-saved map progress for", player.Name)
	else
		warn("[PlayerData] Failed to serialize player data for", player.Name)
	end
...
-- Add tower to specific loadout slot
function PlayerDataManager.AddToLoadout(player, towerId, slotNumber)
	local playerData = player:FindFirstChild("PlayerData")
	...
	-- Check if tower is already in loadout (in another slot)
	for _, slot in ipairs(loadout:GetChildren()) do
		if slot:IsA("StringValue") and slot.Value == towerId then
			warn("[PlayerData] Tower already in loadout:", towerId)
			return false, "Tower already in loadout"
		end
	end
	
	-- Remove existing tower in this slot if any
	...
	towerValue.Value = towerId
	towerValue.Parent = loadout
	
	print("[PlayerData] Added", towerId, "to", player.Name, "'s loadout slot", slotNumber)
	return true, slotNumber
end

-- Remove tower from loadout (by tower ID or slot number)
function PlayerDataManager.RemoveFromLoadout(player, towerIdOrSlot)
	local playerData = player:FindFirstChild("PlayerData")
	if not playerData then return false end
	...
	if type(towerIdOrSlot) == "number" then
		local slot = loadout:FindFirstChild("Slot_" .. towerIdOrSlot)
		if slot then
			local towerId = slot.Value
			slot:Destroy()
			print("[PlayerData] Removed slot", towerIdOrSlot, "from", player.Name, "'s loadout")
			return true, towerId
		end
	else
		-- It's a tower ID - find and remove it
		for _, slot in ipairs(loadout:GetChildren()) do
			if slot:IsA("StringValue") and slot.Value == towerIdOrSlot then
				local slotNumber = tonumber(slot.Name:match("Slot_(%d+)"))
				slot:Destroy()
				print("[PlayerData] Removed", towerIdOrSlot, "from", player.Name, "'s loadout")
				return true, slotNumber
			end
		end
	end
...
-- Get player's inventory as table
function PlayerDataManager.GetInventory(player)
	...
	local towers = {}
	for _, tower in ipairs(inventory:GetChildren()) do
		if tower:IsA("StringValue") then
			table.insert(towers, {
				InstanceName = tower.Name,
				TowerId = tower.Value
			})
		end
	end
...
function PlayerDataManager.CreateUnitInstance(player, unitId, rarity, traits)
	...
	-- Equipped relics subfolder (empty initially)
	local relicsFolder = Instance.new("Folder")
	relicsFolder.Name = "EquippedRelics"
	relicsFolder.Parent = instanceFolder
...
-- Get all unit instances for a player
...
-- Get unit instances by UnitId (for finding all copies of a unit)
function PlayerDataManager.GetUnitInstancesByUnitId(player, unitId)
	local allInstances = PlayerDataManager.GetAllUnitInstances(player)
	local matching = {}
	
	for instanceId, data in pairs(allInstances) do
		if data.UnitId == unitId then
			matching[instanceId] = data
		end
	end
...
-- Update unit instance XP
function PlayerDataManager.UpdateUnitXP(player, instanceId, xpAmount)
	local playerData = player:FindFirstChild("PlayerData")
	if not playerData then return false end
	...
	instance:SetAttribute("XP", currentXP + xpAmount)
	
	return true
end

-- Update unit instance evolution stage
function PlayerDataManager.UpdateUnitEvolutionStage(player, instanceId, stage)
	local playerData = player:FindFirstChild("PlayerData")
	if not playerData then return false end
...
-- Lock/unlock a unit instance (prevent accidental deletion)
...
-- Delete a unit instance (for selling/sacrificing)
function PlayerDataManager.DeleteUnitInstance(player, instanceId)
	...
	instance:Destroy()
	print("[PlayerData] Deleted unit instance:", instanceId)
	return true
end

-- Get unit count (for inventory limit checks)
...
function PlayerDataManager.AddRelic(player, relicData)
	...
	print("[PlayerData] Added relic:", relicData.ID)
	return true
end

-- Remove relic from inventory
function PlayerDataManager.RemoveRelic(player, relicId)
	local playerData = player:FindFirstChild("PlayerData")
	if not playerData then return false end
	...
	local relic = relicInventory:FindFirstChild(relicId)
	if relic then
		relic:Destroy()
		print("[PlayerData] Removed relic:", relicId)
		return true
	end
...
-- Get relic count
function PlayerDataManager.GetRelicCount(player)
	local playerData = player:FindFirstChild("PlayerData")
	if not playerData then return 0 end
	
	local relicInventory = playerData:FindFirstChild("RelicInventory")
	if not relicInventory then return 0 end
	
	return #relicInventory:GetChildren()
end
